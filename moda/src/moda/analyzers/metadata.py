...
class MetadataAnalyzer(BaseAnalyzer):
    ...
    def __init__(self) -> None:
        ...
        configured = config.get("metadata_patterns", {}).get("suspicious_authors", [])
        self.suspicious_authors = {
            str(item).lower() for item in configured or self.SUSPICIOUS_AUTHORS if str(item)
        }
        ...
        try:
            ...
            if olefile.isOleFile(context.file_bytes):
                with olefile.OleFileIO(context.file_bytes) as ole:
                    ...
                    metadata["LastSavedBy"] = (
                        meta.last_saved_by.decode("utf-8", errors="ignore")
                        if meta.last_saved_by
                        else None
                    )
                    metadata["CreateTime"] = str(meta.create_time) if meta.create_time else None
                    metadata["LastSavedTime"] = (
                        str(meta.last_saved_time) if meta.last_saved_time else None
                    )
                    metadata["CreatingApplication"] = (
                        meta.creating_application.decode("utf-8", errors="ignore")
    ...
    def _extract_pdf_metadata(self, context: AnalysisContext) -> dict:
        ...
        try:
            ...
            context.errors.append(f"PDF metadata extraction failed: {e}")
    ...
    def _flag_suspicious_metadata(self, context: AnalysisContext, metadata: dict) -> None:
        ...
        if (
            metadata.get("CreateTime") == metadata.get("LastSavedTime")
            and metadata.get("CreateTime") is not None
        ):
